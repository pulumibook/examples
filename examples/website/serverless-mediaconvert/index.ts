...
const convertRole = new aws.iam.Role("convert-role", {
    ...
    managedPolicyArns: [aws.iam.ManagedPolicy.AmazonS3FullAccess],
});

// Handle uploads by extracting the video filename and creating a new MediaConvert job.
inputBucket.onObjectCreated(
    "handler",
    new aws.lambda.CallbackFunction("handler", {
        policies: [
        ...
        callback: async (event: aws.s3.BucketEvent) => {
            ...
            const client = new MediaConvert({ region });
            ...
            const jobRequest = await new MediaConvert({ endpoint })
                .createJob({
